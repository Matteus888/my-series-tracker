...
function SeasonBlock({ seasonNumber, episodes, onToggle }) {
  ...
  const allWatched = watchedCount === total;
    ...
    <div className={styles.season}>
      ...
      {open && (
        <ul className={styles.episodeList}>
          {episodes.map((ep) => (
            <li key={ep._id ?? `${ep.seasonNumber}-${ep.episodeNumber}`} className={styles.episodeRow}>
              <button
                className={`btn check ${styles.checkButton} ${ep.watched ? "active" : ""}`}
                onClick={() => onToggle(ep._id, ep.watched, ep.seasonNumber, ep.episodeNumber)}
                title={
                  !ep._id ? "Track this series to mark episodes" : ep.watched ? "Mark as unwatched" : "Mark as watched"
                }
              >
